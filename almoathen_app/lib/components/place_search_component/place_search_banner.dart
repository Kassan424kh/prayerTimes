...
import 'package:almoathen_app/components/place_search_component/place_search_page.dart';
...
class PlaceSearchBanner extends StatefulWidget {
  ...
  const PlaceSearchBanner({Key key, this.backgroundImageBlurEffect}) : super(key: key);
  ...
  setAppSettings() async {
    timer = Timer.periodic(new Duration(seconds: 1), (timer) {
      Provider.of<AppSettingsProvider>(context).getAppSettings().then((Map<String, dynamic> appSettings) async {
        if (appSettings != null) {
          Map<String, dynamic> appSettingsObject = appSettings;
          String place = appSettingsObject != null ? appSettingsObject["place"]["place"] : "";
          String placeN = place.length != 0 ? place.length > 15 ? place.substring(0, 15) + "..." : place : "No selected place";
          setState(() {
            placeName = placeN;
          });
          timer.cancel();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black12, offset: Offset(0, 10), blurRadius: 30)],
