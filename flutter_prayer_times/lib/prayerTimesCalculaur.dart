...
class PrayerTimesCalculatur {
  static Map PrayTimes(method) {
    // Time Names
    Map<String, String> timeNames = {
      'imsak': 'Imsak',
      'fajr': 'Fajr',
      'sunrise': 'Sunrise',
      'dhuhr': 'Dhuhr',
	...
	*/
    ...
    List<String> timeSuffixes = ['am', 'pm'];
    String invalidTime = '-----';
    ...
    var lat, lng, elv, // coordinates
        timeZone, jDate; // time variables


    //---------------------- Initialization -----------------------
    // set methods defaults
    Map<String, dynamic> defParams = defaultParams;
    for (var i in methods.keys) {
      var params = methods[i]['params'];
      for (var j in defParams.keys)
    ...
    return {
      ...
      'getTimes': (date, coords, timezone, dst, String format) {
        lat = 1 * coords[0];
        lng = 1 * coords[1];
      ...
      'getFormattedTime': (time, format, suffixes) {
        ...
        var hour = (format == '24h') ? this.twoDigitsFormat(hours) : ((hours +
            12 - 1) % 12 + 1);
      ...
      'computePrayerTimes': (times) {
        ...
        var imsak = this.sunAngleTime(
            this.eval(params.imsak), times.imsak, 'ccw');
        var fajr = this.sunAngleTime(this.eval(params.fajr), times.fajr, 'ccw');
        var sunrise = this.sunAngleTime(
            this.riseSetAngle(), times.sunrise, 'ccw');
        var dhuhr = this.midDay(times.dhuhr);
        var asr = this.asrTime(this.asrFactor(params.asr), times.asr);
        var sunset = this.sunAngleTime(this.riseSetAngle(), times.sunset);
        ;
        var maghrib = this.sunAngleTime(
            this.eval(params.maghrib), times.maghrib);
      ...
      'adjustHighLats': (times) {
        ...
        times.imsak = this.adjustHLTime(
            times.imsak, times.sunrise, this.eval(params.imsak), nightTime,
            'ccw');
        times.fajr = this.adjustHLTime(
            times.fajr, times.sunrise, this.eval(params.fajr), nightTime,
            'ccw');
        times.isha = this.adjustHLTime(
            times.isha, times.sunset, this.eval(params.isha), nightTime);
        times.maghrib = this.adjustHLTime(
            times.maghrib, times.sunset, this.eval(params.maghrib), nightTime);
      ...
      'gmtOffset': (date) {
        var localDate = new Date(
            date[0], date[1] - 1, date[2], 12, 0, 0, 0);
